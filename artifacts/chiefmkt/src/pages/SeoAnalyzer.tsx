...
import { Search, AlertTriangle, AlertCircle, Info, ExternalLink, RefreshCw } from "lucide-react";
...
export default function SeoAnalyzer() {
  ...
  const handleAnalyze = (e: React.FormEvent) => {
    e.preventDefault();
    if (!url) return;
    analyze({ data: { url, projectId: PROJECT_ID } });
  };
      ...
      {!analysis && !isPending && history && history.length > 0 && (
        <div className="pt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {history.map(report => (
              <div key={report.id} className="p-4 glass-panel rounded-xl flex justify-between items-center group cursor-pointer hover:border-primary/50 transition-colors" onClick={() => { setUrl(report.url); analyze({ data: { url: report.url, projectId: PROJECT_ID }}); }}>
